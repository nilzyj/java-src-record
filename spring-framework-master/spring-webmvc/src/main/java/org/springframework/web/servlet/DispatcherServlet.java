...
/**
 * handlers/controllers，HTTP请求的核心分发器
...
public class DispatcherServlet extends FrameworkServlet {
	 ...
	 * 可能在子类中会覆盖该方法为了初始化更深层次的策略对象
	 * <p>May be overridden in subclasses in order to initialize further strategy objects.
	 * 该方法在WebApplicationContext初始化后自动执行，自动扫描上下文的Bean，根据名称或类型匹配的机制查找自定义的组件，
	 * 如果没有找到则会装配一套Spring的默认组件（此处可以更深入了解）。默认组件在DispatcherServlet.properties中指定。
	 */
	protected void initStrategies(ApplicationContext context) {
		...
		initViewResolvers(context); // 初始化视图解析器，解析逻辑视图名到具体视图实现
	 ...
	 * 将请求转换为multipart请求，并使multipart多部分解析器可用
	...
	protected HttpServletRequest checkMultipart(HttpServletRequest request) throws MultipartException {
		// 判断是否配置multipartResolver，判断请求中是否包含文件
